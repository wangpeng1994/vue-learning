...
var appTodo = new _vue2.default({
  ...
  methods: {
    fetchTodos: function fetchTodos() {
      ...
      if (this.currentUser) {
        ...
        query.find().then(function (todos) {
          ...
          _this.todoList.id = id; //在页面刚载入的时候由于还没出现save(create)操作，尽管远程数据表可能存在并有id，但是todoList上是没有id的，而 add 和 delete 都会调用 saveOrUpdateTodos 判断是否有id，所以这里就需要挂在可能存在id，后面如果有 save(create)操作，间接说明这里挂了id = null，也因此需要再次挂载 id，以便为接下来的save(create) 或者 update提供依据
        }, function (error) {
          console.error(error);
        });
      }
    ...
    saveTodos: function saveTodos() {
      ...
      var dataString = JSON.stringify(this.todoList);
      var AVTodos = _leancloudStorage2.default.Object.extend('AllTodos'); //声明一个 AllTodos 表类型
